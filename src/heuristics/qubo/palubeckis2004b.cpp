...
bool Palubeckis2004bSolution::STSMod(double *best_objective, int zmax,
  ...
  for (int iter=0; z < zmax; ++iter) {
	...
	*best = *this;
	...
	return 0;  // Out of time or termination criterion
      }
    }
...
Palubeckis2004bMST4::Palubeckis2004bMST4(const QUBOInstance& qi,
  ...
  while (true) {
    ...
    if (!initial.STSMod(&best_objective, zmax, &B, mStar, Delta)) {
